...
- 大厅、设置和规则页支持竖屏；牌桌在竖屏时提示旋转至横屏，并适配电脑、横屏手机和横屏平板。
...
- 2–10 人无限注德州扑克（总人数包含房主，即 1 名房主 + 最多 9 名远端玩家）：
  - 按钮、小盲、大盲与单挑盲注顺序；
  - 翻牌前、翻牌、转牌、河牌四轮下注；
  - 过牌、下注、跟注、加注、弃牌、全下；
...
- 请允许浏览器访问本地网络，并在生成、扫描二维码和连接完成前保持两端页面处于前台。
- ICE 信息收集或连接超过时限时，应用会停止等待并显示重试提示；请生成新邀请，不要重复使用旧二维码。
- 不配置公网 STUN/TURN 或信令服务器，因此设计目标是同一 Wi‑Fi/热点，而不是互联网远程联机。
- 房主页面负责发牌、校验、计时和结算。房主关闭应用、系统冻结后台页面或离开牌桌会结束当前联机。
- 每局支持 2–10 名玩家，总计包含房主；每位远端玩家都要独立完成一次双向扫码。
- 规则引擎和座位布局支持 10 人，但 1 名房主连接 9 台远端设备尚未经过持续满载实机验证，实际稳定性取决于房主设备、浏览器和局域网。
...
规则状态机位于 [`src/game/engine.ts`](src/game/engine.ts)，牌型比较器位于 [`src/game/evaluator.ts`](src/game/evaluator.ts)，WebRTC 房间位于 [`src/services/webrtc.ts`](src/services/webrtc.ts)。测试覆盖牌型顺序、A‑2‑3‑4‑5 顺子、底牌隐藏、单挑盲注、弃牌结算、边池守恒、最小加注、短码全下不重新开放加注，以及配对码压缩/校验、完整 ICE 收集、连接超时和 guest 入座握手。
